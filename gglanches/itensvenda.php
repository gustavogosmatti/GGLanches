 ...
 */
    ...
    class itensvendaPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset = new QueryDataset(
              ...
              $selectQuery, $insertQuery, $updateQuery, $deleteQuery, 'itensvenda');
        ...
        protected function getFiltersColumns()
        {
            return array(
                ...
                new FilterColumn($this->dataset, 'itemsub', 'itemsub', 'Itemsub')
            );
        }
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            ...
            //
            $column = new NumberViewColumn('itemsub', 'itemsub', 'Itemsub', $this->dataset);
            $column->SetOrderable(true);
            $column->setNumberAfterDecimal(2);
            $column->setThousandsSeparator('.');
            $column->setDecimalSeparator(',');
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for itemsub field
            //
            $editor = new TextEdit('itemsub_edit');
            $editColumn = new CustomEditColumn('Itemsub', 'itemsub', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $editColumn->GetCaption()));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
    ...
    try
    {
        $Page = new itensvendaPage("itensvenda", "itensvenda.php", GetCurrentUserPermissionSetForDataSource("itensvenda"), 'UTF-8');
        $Page->SetTitle('Itensvenda');
        $Page->SetMenuLabel('Itensvenda');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("itensvenda"));
